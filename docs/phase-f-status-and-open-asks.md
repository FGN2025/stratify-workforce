...
3. ✅ **T0 ping SENT (2026-05-11) — Play ACK received, shadow live.** First dispatched webhooks landed `2026-05-11 23:09:13.659 UTC` and `23:09:18.333 UTC` (`play_sync_attempts` ids `2cc8b893…` / `f6163815…`) for **CS - Bronze Challenge** (`external_attempt_id=dbd3fc50-60b0-4cba-aa18-06cb7cb93fd5`, RacerX/`darcy@fgn.gg`, tenant `acme-broadband`). Both dispatched in `sig_mode=lenient`, `status=completed`. **Parity window anchor T0_dispatch = 2026-05-11 23:09:13.659 UTC; close = 2026-05-13 23:09 UTC (48h).**
   ...
   - ✅ **T0+~3.7h re-fire (2026-05-12 02:54:45 UTC) — canonical-resolution path validated on the wire.** Single `play_sync_attempts` row (`dfeb403a…`) for the same `external_attempt_id=dbd3fc50…` (CS - Bronze, RacerX/`darcy@fgn.gg`, tenant `acme-broadband`). Wire bytes confirm all four Play-side fixes landed:
     - `payload.delivery_id=dbd3fc50…` at top-level ✅ + `payload.metadata.delivery_id=dbd3fc50…` ✅ (both PR P-3 slots populated; receiver headers extension already deployed)
     - `payload.score=100`, `payload.metadata.awarded_points=20`, `payload.metadata.max_points=20` — canonical resolution holding, no Retry-button 0-clobber ✅
     - `payload.completed_at=2026-05-11T23:09:04.280168+00:00` — byte-identical to canonical `challenge_completions.completed_at`, no `now()` drift ✅
     - `sig_mode=lenient` (HMAC primitive 0 mismatches across both firings) ✅
     - Receiver outcome: `attempt_number=2`, `xp_awarded=20`, `status=completed`, credential `a7494e83…` issued cleanly. No flip-to-failed, no duplicate row.
     - **Net:** Flag 1 closed at root cause (canonical resolution from `challenge_completions` works); Flag 3 closed (delivery_id surface live end-to-end). Flag 2 (direct-POST repoint) — no second `play_sync_attempts` row this firing, so direct path either wasn't fired this time or was idempotency-collapsed; will confirm on next moderator-approval firing whether dispatch+direct now both land.
