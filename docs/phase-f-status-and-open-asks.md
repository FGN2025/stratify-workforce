...
3. ✅ **T0 ping SENT (2026-05-11) — Play ACK received, shadow live.** First dispatched webhooks landed `2026-05-11 23:09:13.659 UTC` and `23:09:18.333 UTC` (`play_sync_attempts` ids `2cc8b893…` / `f6163815…`) for **CS - Bronze Challenge** (`external_attempt_id=dbd3fc50-60b0-4cba-aa18-06cb7cb93fd5`, RacerX/`darcy@fgn.gg`, tenant `acme-broadband`). Both dispatched in `sig_mode=lenient`, `status=completed`, score 100 → 0 between the two firings (Play re-emit / score correction — flagged for parity diff review). **Parity window anchor T0_dispatch = 2026-05-11 23:09:13.659 UTC; close = 2026-05-13 23:09 UTC (48h).**
   - ⚠️ **Parity gap noted:** no paired direct `challenge_completion` row in `play_sync_attempts` for the same `external_attempt_id` — shadow expects both fire; either Play is dispatch-only already or the direct path isn't logging. Asking Play to confirm.
   - ⚠️ **Idempotency gap:** outer envelope `delivery_id` is null on both firings → receiver can't dedupe re-emits; `external_attempt_id` lives in `payload.metadata` only. Hardening tracked under PR P-3 (delivery_id header/envelope).
