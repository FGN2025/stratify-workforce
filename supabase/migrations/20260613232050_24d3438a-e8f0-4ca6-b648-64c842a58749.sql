CREATE OR REPLACE FUNCTION public.backfill_credentials_for_course(p_course_id uuid)
 RETURNS TABLE(minted_count integer, skipped_count integer, completions_scanned integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  FOR r IN
    SELECT DISTINCT
      ...
      uwc.score,
      uwc.completed_at,
      wo.fgn_origin_challenge_id AS challenge_id,
    ...
    JOIN public.challenge_lesson_mappings clm
      ON clm.play_challenge_id = wo.fgn_origin_challenge_id
