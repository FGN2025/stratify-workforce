...
export const CREDENTIAL_API: ApiSection = {
  ...
  endpoints: [
    ...
    },
    {
      ...
      description: 'Search and filter credentials across all users with pagination. Requires an authorized app API key with read permission.',
      auth: 'apikey',
      tags: ['Authorized', 'Credentials', 'Search'],
      parameters: [
        { name: 'game_title', location: 'query', type: 'string', required: false, description: 'Filter by game title', enum: ['ATS', 'Farming_Sim', 'Construction_Sim', 'Mechanic_Sim', 'Fiber_Tech'] },
        { name: 'type', location: 'query', type: 'string', required: false, description: 'Filter by credential type key', example: 'cdl-pre-trip' },
        { name: 'issued_after', location: 'query', type: 'string', required: false, description: 'ISO 8601 date — only credentials issued after this date', example: '2025-01-01' },
        { name: 'issued_before', location: 'query', type: 'string', required: false, description: 'ISO 8601 date — only credentials issued before this date', example: '2025-12-31' },
        { name: 'page', location: 'query', type: 'integer', required: false, description: 'Page number (default: 1)', example: '1' },
        { name: 'page_size', location: 'query', type: 'integer', required: false, description: 'Results per page, max 100 (default: 50)', example: '50' },
      ],
      responses: [
        { status: 200, description: 'Paginated credential results', example: { credentials: [], pagination: { page: 1, page_size: 50, total: 0, total_pages: 0 } } },
        { status: 401, description: 'Invalid API key' },
        { status: 403, description: 'App does not have read permission' },
      ],
    },
    {
      ...
      description: 'Register a webhook URL to receive event notifications. Returns a signing secret for HMAC-SHA256 payload verification.',
      auth: 'apikey',
      tags: ['Authorized', 'Webhooks'],
      parameters: [],
      requestBody: {
        description: 'Webhook registration payload',
        example: { webhook_url: 'https://your-app.com/webhooks/fgn', events: ['credential.issued', 'work_order.completed'], secret: 'optional-your-own-secret' },
      },
      responses: [
        { status: 201, description: 'Webhook created with signing secret', example: { webhook: { id: '...', events: ['credential.issued'] }, secret: 'hex-string', note: 'Store this secret securely.' } },
        { status: 400, description: 'Invalid webhook_url or events' },
        { status: 401, description: 'Invalid API key' },
      ],
    },
    {
      ...
      path: '/webhooks',
      title: 'List Webhooks',
      description: 'List all webhook subscriptions for the authenticated app.',
      auth: 'apikey',
      tags: ['Authorized', 'Webhooks'],
      parameters: [],
      responses: [
        { status: 200, description: 'List of webhooks', example: { webhooks: [] } },
        { status: 401, description: 'Invalid API key' },
      ],
    },
    {
      ...
      description: 'Remove a webhook subscription. Stops all future event deliveries.',
      auth: 'apikey',
      tags: ['Authorized', 'Webhooks'],
      parameters: [
        { name: 'id', location: 'path', type: 'uuid', required: true, description: 'Webhook subscription ID' },
      ],
      responses: [
    ...
    },
    {
      ...
      description: 'View delivery history for a specific webhook subscription.',
      auth: 'apikey',
      tags: ['Authorized', 'Webhooks'],
      parameters: [
        { name: 'id', location: 'path', type: 'uuid', required: true, description: 'Webhook subscription ID' },
        { name: 'page', location: 'query', type: 'integer', required: false, description: 'Page number', example: '1' },
        { name: 'page_size', location: 'query', type: 'integer', required: false, description: 'Results per page', example: '20' },
      ],
      responses: [
        { status: 200, description: 'Delivery log entries', example: { deliveries: [], pagination: { page: 1, page_size: 20, total: 0 } } },
