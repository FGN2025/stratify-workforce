  ...
  const handleImageSelect = async (url: string) => {
    try {
      ...
      toast({
        ...
        description: 'The course cover has been changed successfully.',
      });
    } catch (error) {
      console.error('Error updating course cover:', error);
      toast({
        title: 'Update failed',
        description: 'Could not update the course cover image.',
        variant: 'destructive',
      });
    }
  };
  ...
  return (
    <>
      <Card className="group overflow-hidden border-border/50 hover:border-primary/50 transition-all duration-300 hover:shadow-lg hover:shadow-primary/5">
        {/* Cover Image */}
        <EditableImageWrapper onEdit={() => setIsPickerOpen(true)}>
